module map_bitset_int32
    ...
    type, public :: bitset_to_int32_map_type
    ...
    contains
        ...
        !* initialize the instance of `bitset_to_int32_map_type`.
        procedure, public, pass :: finalize
        !* finalize the instance of `bitset_to_int32_map_type`.
    end type bitset_to_int32_map_type
...
contains
    ...
    subroutine put(this, key, value, status)
        ...
        implicit none
        class(bitset_to_int32_map_type), intent(inout) :: this
            !! passed-object dummy argument
        class(bitset_type), intent(in) :: key
        ...
        ! bitset_type instances do not have the same memory values
        ! even though they have the same bitset.
        ! So the bitset value as the string is used as the key.
        character(:), allocatable :: key_str
        call key%to_string(key_str)

        call append(this%map, key_str, value, status)
    ...
    function get_value(this, key, status) result(val)
        ...
        call get_other(this%map, key_str, other, status)
        if (error_occurred(status)) return
    ...
    function contains_key(this, key, status)
        ...
        call this%map%key_test(to_hash_key(key_str), contains_key)
        call set_success(status)
    ...
    subroutine remove_key(this, key, status)
        ...
        call this%map%remove(to_hash_key(key_str), existed)
        if (.not. existed) then
    ...
    !>Finalizes the instance of `bitset_to_int32_map_type`.
